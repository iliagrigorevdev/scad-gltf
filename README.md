...
- **Automated AI Generation:** Pass an OpenAI-compatible API key or base URL to the `scad-godot` or `scad-web` CLI to automatically spawn a local MCP server, query the LLM via standard endpoints, allow it to visually iterate, and output a ready-to-run project generator script.
