...
std::shared_ptr<PolySet> ManifoldGeometry::toPolySet() const
{
  ...
  auto getFaceFrontColorIndex = [&]() -> int {
    if (faceFrontColorIndex < 0) {
      faceFrontColorIndex = ps->materials.size();
      MaterialProperties mat;
      mat.color = ColorMap::getColor(*colorScheme, RenderColor::CGAL_FACE_FRONT_COLOR);
  ...
  auto getFaceBackColorIndex = [&]() -> int {
    if (faceBackColorIndex < 0) {
      faceBackColorIndex = ps->materials.size();
      MaterialProperties mat;
      mat.color = ColorMap::getColor(*colorScheme, RenderColor::CGAL_FACE_BACK_COLOR);
      ps->materials.push_back(mat);
    }
    return faceBackColorIndex;
  };

  std::map<MaterialProperties, int32_t> materialToIndex;
  ...
  auto getColorIndex = [&](uint32_t originalID) -> int32_t {
    ...
    auto mapIt = materialToIndex.find(mat);
    if (mapIt != materialToIndex.end()) {
      originalIDToColorIndex[originalID] = mapIt->second;
      return mapIt->second;
    } else {
      int32_t color_index = ps->materials.size();
      ps->materials.push_back(mat);
      materialToIndex[mat] = color_index;
                                         ...
                                         manifold::OpType opType) const
{
  ...
  if (opType == manifold::OpType::Subtract) {
    ...
    for (const auto id : rhs.originalIDs_) {
      auto it = rhs.materials_.find(id);
      if (it != rhs.materials_.end()) {
        materials[id] = it->second;
      } else {
  ...
  return {mani, originalIDs, materials, subtractedIDs};
}
...
void ManifoldGeometry::setColor(const MaterialProperties& properties)
{
